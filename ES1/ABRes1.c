...
ARB* searchConditionAndDeleteABR(ARB*, char*, char*);
...
ARB* searchConditionAndDeleteABR(ARB* ROOT, char* strMin, char* strMax){
    if(ROOT!=NULL){
        if(strcmp(ROOT->stringElement, strMin)>=0 && strcmp(ROOT->stringElement, strMax)<=0){
            if(strcmp(ROOT->stringElement, strMin)>0){
                ROOT->sx=searchConditionAndDeleteABR(ROOT->sx, strMin, strMax);
            }
            if(strcmp(ROOT->stringElement, strMax)<0){
                ROOT->dx=searchConditionAndDeleteABR(ROOT->dx, strMin, strMax);
            }
            if(strlen(ROOT->stringElement)%2==0){
                ROOT=adjustARB(ROOT);
            }
        }else{
            if(strcmp(ROOT->stringElement, strMin)>0){
...
/***
7. costruzione di un ABR contenente un numero n di stringhe casualmente generate, dove n
...
//PER BILANCIARE è SUFFICIENTE RIEMPIRE LA RADICE DELL'ALBERO CON L'ELEMENTO MEDIANO DEL VETTORE ORDINATO
