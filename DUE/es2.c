...
ARB* casualARB(ARB* ROOT, int numNodes, FUNRAND frandom, FUNCOM compare, FUNCPY fcopy){
    srand(time(NULL));
    void* toIns;
    while(numNodes>0){
        toIns=frandom();
        ROOT=insertARBNode(ROOT, toIns, compare, fcopy);
        numNodes--;
    }
    return ROOT;
}
...
ARB* duplicateARB(ARB* original, FUNCPY fcopy){
    ...
    if(original!=NULL){
        ...
        copy->element=fcopy(copy->element, original->element);
...
void** vectorizeFunction(ARB* ROOT, void** vec, int* k, FUNCPY fcopy){
    if(ROOT!=NULL){
            //RICORRE FINO A RAGGIUNGERE UN NODO NULLO
		vec=vectorizeFunction(ROOT->dx, vec, k, fcopy);
    //SCENDE A DESTRA
		//vec[(*k)]=(char*)malloc(sizeof(char)*strlen(ROOT->stringElement));
		fcopy(vec[(*k)],ROOT->element);
		(*k)=(*k)-1;
		...
		vec=vectorizeFunction(ROOT->sx, vec, k, fcopy);
		//SCENDE A SINISTRA
	}
	return vec;
}
...
ARB* balanceFunction(void** source, int i, int j, ARB* res, FUNCOM compare, FUNCPY fcopy){
    int k;
    if(i<=j){
        k=(i+j)/2;
        res=insertARBNode(res, source[k], compare, fcopy);
        res=balanceFunction(source, i, k-1, res, compare, fcopy);
        res=balanceFunction(source, k+1, j, res, compare, fcopy);
    }
    return res;
}
...
ARB* balanceARB(ARB* toBalance, FUNCOM compare, FUNCPY fcopy){
    ...
    result=balanceFunction(temp, 0, size-1, result, compare, fcopy);
    for(i=0; i<size; i++){
        free(temp[i]);
    }
    free(temp);
